"""Walk-Forward Backtest v1.5 — Asimetrik R/R (TP=ATR×2, SL=ATR×1)."""
import os
import sys
import time
import numpy as np
...
TEST_DAYS = 1
N_WINDOWS = 3
# === v1.5 ASİMETRİK R/R KONFİG ===
CONFIDENCE_THRESHOLD = 0.65   # 0.55 → 0.65 (sıkı filtre)
FEE_RATE = 0.0004
HOLD_PERIOD_MINUTES = 30      # 1 → 30 (fee'yi seyret)
MIN_TRAIN_SAMPLES = 500

# ATR-based exit
TP_ATR_MULT = 2.0   # Take profit = entry ± ATR × 2.0
SL_ATR_MULT = 1.0   # Stop loss   = entry ∓ ATR × 1.0
# R/R = 2:1 (asimetrik) — %50 hit rate bile pozitif EV verir
...
def run_window(df, window):
    ...
    test_high = test_clean["high"].values
    test_low = test_clean["low"].values
    test_atr = test_clean["atr_14"].values
    signal_pnls = []
    signal_hits = []   # TP=1, SL=0, TIMEOUT=net>0?1:0
    outcomes = {"TP": 0, "SL": 0, "TIMEOUT_WIN": 0, "TIMEOUT_LOSS": 0}
    ...
    for idx in np.where(signals_mask)[0]:
        ...
        if np.isnan(atr) or atr <= 0:
            continue
        direction = 1 if pred[idx] == 1 else -1  # LONG=1, SHORT=-1
        ...
        # 30 dakikalık pencerede mum mum izle
        outcome = None
        exit_price = entry_price
        for step in range(1, HOLD_PERIOD_MINUTES + 1):
            if idx + step >= len(test_close):
                break
            h = test_high[idx + step]
            l = test_low[idx + step]
            if direction == 1:  # LONG
                # Önce SL (konservatif: aynı mumda ikisi de vurulursa SL kazanır)
                if l <= sl_price:
                    outcome = "SL"
                    exit_price = sl_price
                    break
        ...
        if outcome is None:
            outcome = "TIMEOUT"
            exit_idx = min(idx + HOLD_PERIOD_MINUTES, len(test_close) - 1)
            exit_price = test_close[exit_idx]
        ...
        # Hit ve outcome sayımı
        if outcome == "TP":
            signal_hits.append(1)
            outcomes["TP"] += 1
        elif outcome == "SL":
            signal_hits.append(0)
            outcomes["SL"] += 1
        else:  # TIMEOUT
            if net_return > 0:
                signal_hits.append(1)
                outcomes["TIMEOUT_WIN"] += 1
            else:
                signal_hits.append(0)
                outcomes["TIMEOUT_LOSS"] += 1
    if not signal_pnls:
    ...
    return {"window_id": window["window_id"], "status": "ok",
            ...
            "n_signals": n_signals, "n_executed": len(signal_pnls),
...
def main():
    print("=" * 70)
    print("WALK-FORWARD BACKTEST v1.5 — Asimetrik R/R 2:1")
    print("=" * 70)
    ...
    for symbol in SYMBOLS:
        ...
        for w in windows:
            ...
            if result["status"] == "ok":
                o = result['outcomes']
                print(f"    Pencere {result['window_id']}: signal={result['n_signals']}/{result['n_executed']} exec, hit={result['hit_rate']*100:.1f}%, PnL={result['total_return_pct']:+.2f}%, PF={result['profit_factor']:.2f}, [TP:{o['TP']} SL:{o['SL']} TIMEOUT:{o['TIMEOUT_WIN']+o['TIMEOUT_LOSS']}] ({time.time()-t1:.1f}s)")
            else:
