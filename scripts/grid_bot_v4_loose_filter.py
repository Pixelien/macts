...
def predict_regime(model, features, available_features):
    ...
    YENİ: predict_proba kullan. Ranging probability >= 0.40 ise RANGING kabul et
    (azınlıkta bile olsa). Bu, grid'in daha çok aktif olmasını sağlar.
    """
    if model is None:
        return None
    ...
    try:
        ...
        # Ranging class index'i (0)
        ranging_idx = classes.index(0) if 0 in classes else None
        if ranging_idx is not None:
            ranging_prob = probs[ranging_idx]
            # Eğer ranging probability >= 0.40 ise RANGING tahmin et
            if ranging_prob >= 0.40:
                return 0
        # Aksi halde en yüksek probability'li sınıf
...
def backtest_grid_with_ml(df, symbol):
    ...
    for i in range(start_idx, len(df)):
        ...
        # YENİ: TRENDING tespit edildiyse VE inventory varsa → likide et
        if not grid_active and inventory_qty > 0 and current_regime is not None:
            # Mevcut inventory'i piyasa fiyatından sat (panik çıkış)
            gross_proceeds = inventory_qty * price
            fee = gross_proceeds * FEE_RATE
            cash += gross_proceeds - fee
            avg_cost = inventory_cost / inventory_qty
            pnl_realized = inventory_qty * (price - avg_cost) - fee
            cycle_pnls.append(pnl_realized)
            inventory_qty = 0.0
            inventory_cost = 0.0
            total_fees_paid += fee
            trade_count += 1
